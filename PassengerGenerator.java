...
class PassengerGenerator extends Thread {
    ...
    public void run() {
        if (this.current_time == next_arrival_time) {
            ...
            System.out.println(passenger_id + " passenger generated at " + this.current_time);
            // System.out.println(new_passenger);

            Random random = new Random();
            int officer_number = random.nextInt(3);
            if (officer_number == 0) {
                queue1.addPassenger(new_passenger, Integer.MAX_VALUE);
                System.out.println("passenger added q1");
            }
            else if (officer_number == 1) {
                queue2.addPassenger(new_passenger, Integer.MAX_VALUE);
                System.out.println("passenger added q2");
            }
            else {
                queue3.addPassenger(new_passenger, Integer.MAX_VALUE);
